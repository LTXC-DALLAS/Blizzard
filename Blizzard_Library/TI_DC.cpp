...
void TI_DC::TestData (IntS row) {
    ...
    if (TI_DC::force_type == OSPins) {
        row_test_result[row] = TIDlog.FloatValue (row_results[row], test_pins[row], opens_limits[row], use_testware, testware_datatype[row], do_clo);
        TMResultM shorts_result;
        shorts_result = TIDlog.FloatValue(row_results[row], test_pins[row], shorts_limits[row], use_testware, testware_datatype[row], do_clo);
        AccumulateResults(row_test_result[row], shorts_result);
    }
    else {
